...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        for downsample_block in self.down_blocks:
            if (
                ...
                # print(sample.shape, down_intrablock_additional_residuals[0].shape)
                if  down_intrablock_additional_residuals is not None and len(down_intrablock_additional_residuals) > 0:
        ...
        if down_block_additional_residuals is not None:  #down_block_additional_residuals , list,  length 12
            new_down_block_res_samples = ()

            for down_block_res_sample, down_block_additional_residual in zip(
                down_block_res_samples, down_block_additional_residuals
            ):
                # print(down_block_res_sample.shape, down_block_additional_residual.shape)
    ...
    def from_pretrained_2d(
        ...
        if flag:
            # drop conv in
            state_dict = {k: v for k, v in state_dict.items() if not k.startswith("conv_in")}
